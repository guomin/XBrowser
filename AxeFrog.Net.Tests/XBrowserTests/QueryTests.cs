...
*/
		...
		public void Test_AllSelector()
		{
			...
			var elements = XQuery.Execute("*", window.Document.XDocument);
			Assert.AreEqual(window.Document.XDocument.Descendants().Count(), elements.Length, "All elements in the document should have been returned");
		}
		...
		{
			var window = GetWindow(TestHtml.Basic2);
			var elements = XQuery.Execute("li", window.Document.XDocument);
		...
		{
			var window = GetWindow(TestHtml.Basic2);
			var elements = XQuery.Execute("#link", window.Document.XDocument);
		...
		public void Test_ChildSelector()
		{
			...
			var elements = XQuery.Execute("body > ul > li", window.Document.XDocument);
		...
		{
			var window = GetWindow(TestHtml.Basic2);
			var elements = XQuery.Execute("ul li", window.Document.XDocument);
		...
		public void Query_starting_or_ending_with_transposal_selector_should_throw_exception()
		{
			...
			try { var elements = XQuery.Execute("> body", window.Document.XDocument); }
		...
		{
			var window = GetWindow(TestHtml.Basic2);
			var elements = XQuery.Execute("ul, ol", window.Document.XDocument);
			Assert.AreEqual(3, elements.Length, "Exactly 3 elements should have been returned");
